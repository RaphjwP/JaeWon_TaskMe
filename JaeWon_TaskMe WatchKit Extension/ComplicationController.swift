...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getComplicationDescriptors(handler: @escaping ([CLKComplicationDescriptor]) -> Void) {
        let descriptors = [
            CLKComplicationDescriptor(identifier: "complication", displayName: "JaeWon_TaskMe", supportedFamilies: CLKComplicationFamily.allCases)
    ...
    func getCurrentTimelineEntry(for complication: CLKComplication, withHandler handler: @escaping (CLKComplicationTimelineEntry?) -> Void) {
        ...
        let iconImage = UIImage(systemName: "bolt" , withConfiguration: UIImage.SymbolConfiguration(pointSize: 35, weight: .regular, scale: .medium))?.withTintColor(.white)
